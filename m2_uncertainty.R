...
modelM2Exo = forecast::Arima(dlnDatam2[,"M2"], order = c(0,0,0),xreg = cbind(dlnDatam2[,"GS10"],dlnDatam2[,"Close"]))
modelyc = forecast::Arima(dlnDatam2[,"GS10"],order = c(1,0,0))
modelsnp = forecast::Arima(dlnDatam2[,"Close"],order = c(0,0,0)) # What order do we select?
...
# Stick eyogenous variables into M2 model
...
plot(as.ts(data.matrix(dlnDatam2[,"M2"])), type = "l", lwd = 2, las = 1, ylab = "",xlim = c(1,T+tHorizons))
